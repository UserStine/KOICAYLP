...
export default function AdminPinResets(){
  ...
  return <div className="portal-page"><header className="portal-head"><div><p className="portal-eyebrow">{t.admin.badge}</p><h1>{l.title}</h1></div></header>
    {secret&&<div className="pin-secret-card"><strong>{l.tempTitle}</strong><div className="pin-secret-value"><code>{secret.pin}</code><button className="btn btn-sm" onClick={copy}>{l.copy}</button></div><button className="btn btn-secondary btn-sm" onClick={()=>setSecret(null)}>{l.done}</button></div>}
