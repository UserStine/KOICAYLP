...
export default function ApplySector({ track = "public" }) {
  const { t } = useT();
  const a = t.apply;
  const [applicationStatus, setApplicationStatus] = useState(CLOSED_STATUS);
  const selectedTrack = track === "private" ? "private" : "public";
  ...
  const currentTrack = selectedTrack === "private"
  ...
  return (
    <main className="page">
      ...
      <section className="section alt">
        ...
        <div className="dl-grid application-single-track">
          {(() => {
            const formUrl = applicationStatus.forms?.[currentTrack.key] || "";
            return (
              <article className={`dl-card ${closed ? "is-disabled" : ""}`}>
                <div className="dl-icon"><FileIcon /></div>
                <div className="dl-copy"><h3>{currentTrack.title}</h3><p>{currentTrack.text}</p></div>
                <div className="application-actions">
                  <ActionButton disabled={closed || !formUrl || downloadBusy === currentTrack.key} onClick={() => downloadForm(currentTrack.key, formUrl)}>
                    {!closed && <DownloadIcon />}
                    {closed ? a.statusClosedButton : downloadBusy === currentTrack.key ? <><PekoLoader compact /> <span>{a.downloading}</span></> : a.download}
                  </ActionButton>
                </div>
              </article>
            );
