...
export function lexicalScores(query, chunks) {
  const queryTokens = tokenize(query);
  const querySet = new Set(queryTokens);
  const normalizedQuery = clean(query, 700).toLowerCase();

  const expansions = [
    { match: /\bcandidature|postuler|soumettre|dossier|formulaire\b/i, terms: ["apply", "application", "submission", "documents", "form"] },
    { match: /\béligib|admissib|critères?|âge|expérience\b/i, terms: ["eligible", "eligibility", "criteria", "age", "experience"] },
    { match: /\bsecteur public|secteur privé|gouvernement|entrepreneur\b/i, terms: ["track", "public", "private", "sector", "government", "entrepreneur"] },
    { match: /지원|신청|제출|서류|양식/u, terms: ["apply", "application", "submission", "documents", "form"] },
    { match: /자격|지원자격|나이|경력/u, terms: ["eligible", "eligibility", "criteria", "age", "experience"] },
    { match: /공공|민간|정부|창업/u, terms: ["track", "public", "private", "sector", "government", "entrepreneur"] },
