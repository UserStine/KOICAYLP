...
3. Set `REQUIRE_EMAIL_VERIFICATION=true` once every production account has an email address and has been verified.
...
5. Forward application logs to centralized logging with alerts for `login_failed`, `rate_limit`, `csrf_origin_rejected`, API 5xx errors, and AI upstream failures. Do not log passwords, PINs, session cookies, reset tokens, verification tokens, or API keys.
...
8. Configure the transactional mail webhook before enabling registration, email verification, or password reset in production. Reset/verification tokens expire after `AUTH_TOKEN_TTL_MINUTES` and are stored only as SHA-256 digests.
